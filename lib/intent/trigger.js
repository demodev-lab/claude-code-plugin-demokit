...
const INTENT_PATTERNS = [
  ...
  {
    id: 'erd',
    patterns: [/erd|다이어그램|엔티티\s*관계/i, /er\s*다이어그램/i],
    command: '/erd',
  ...
  {
    id: 'optimize',
    patterns: [/최적화|optimize|n\+1|인덱스\s*(추가|분석)/i, /쿼리\s*최적화/i, /성능\s*(분석|개선)/i],
    command: '/optimize',
  ...
  {
    id: 'changelog',
    patterns: [/changelog|변경\s*로그|릴리즈\s*노트/i],
    command: '/changelog',
  ...
  {
    id: 'properties',
    patterns: [/application\.yml|application\.properties|설정\s*파일/i],
    command: '/properties',
...
function matchIntent(prompt) {
  ...
  const isStrongPattern = /crud|스캐폴드|리뷰|review|pdca|loop|루프|커밋|commit|푸시|push|\bpr\b|풀리퀘|erd|최적화|optimize|changelog|변경\s*로그/.test(prompt.toLowerCase());
