/**
 * Loop 상태 관리
 * Ralph Wiggum 스타일 자율 반복 루프의 상태를 파일 기반으로 관리
 *
 * 상태 파일 위치: {projectRoot}/.demodev/loop-state.json
 */
const fs = require('fs');
const path = require('path');
const { ensureDir } = require('../core/io');

const LOOP_DIR = '.demodev';
const STATE_FILENAME = 'loop-state.json';

function getStateFilePath(projectRoot) {
  return path.join(projectRoot, LOOP_DIR, STATE_FILENAME);
}

/**
 * 현재 루프 상태 조회 (프로세스 내 200ms TTL 캐시)
 */
let _stateCache = { projectRoot: null, state: null, expiresAt: 0 };

function getState(projectRoot) {
  const now = Date.now();
  if (_stateCache.projectRoot === projectRoot && now < _stateCache.expiresAt) {
    return _stateCache.state;
  }
  const filePath = getStateFilePath(projectRoot);
  let state = { active: false };
  try {
    if (fs.existsSync(filePath)) {
      state = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
    }
  } catch { /* ignore */ }
  _stateCache = { projectRoot, state, expiresAt: now + 200 };
  return state;
}

function _invalidateCache() { _stateCache.expiresAt = 0; }

/**
 * 루프 시작
 */
function startLoop(projectRoot, { prompt, maxIterations = 10, completionPromise = 'LOOP_DONE' }) {
  // 이전 loop-log.md 아카이브
  try {
    const { writer } = require('../context-store');
    writer.archiveLoopLog(projectRoot);
  } catch { /* context-store 로드 실패 시 무시 */ }

  const filePath = getStateFilePath(projectRoot);
  ensureDir(path.dirname(filePath));
  const state = {
    active: true,
    prompt,
    completionPromise,
    maxIterations,
    currentIteration: 1, // 초기 실행이 1회차 (0이면 off-by-one으로 N+1회 실행됨)
    startedAt: new Date().toISOString(),
  };
  fs.writeFileSync(filePath, JSON.stringify(state, null, 2));
  _invalidateCache();
  return state;
}

/**
 * 반복 횟수 증가
 */
function incrementIteration(projectRoot) {
  const state = getState(projectRoot);
  state.currentIteration = (state.currentIteration || 0) + 1;
  const filePath = getStateFilePath(projectRoot);
  fs.writeFileSync(filePath, JSON.stringify(state, null, 2));
  _invalidateCache();
  return state;
}

/**
 * 루프 완료 표시
 */
function completeLoop(projectRoot) {
  const state = getState(projectRoot);
  state.active = false;
  state.completedAt = new Date().toISOString();
  const filePath = getStateFilePath(projectRoot);
  fs.writeFileSync(filePath, JSON.stringify(state, null, 2));
  _invalidateCache();
  return state;
}

/**
 * 루프 취소 (상태 파일 삭제)
 */
function cancelLoop(projectRoot) {
  const filePath = getStateFilePath(projectRoot);
  try {
    fs.unlinkSync(filePath);
  } catch (err) {
    if (err.code !== 'ENOENT') throw err;
  }
  _invalidateCache();
}

/**
 * max iterations 도달 여부
 */
function isMaxReached(state) {
  return state.active && state.currentIteration >= state.maxIterations;
}

/**
 * Rate limit 기록
 */
function recordRateLimit(projectRoot) {
  const state = getState(projectRoot);
  state.rateLimitCount = (state.rateLimitCount || 0) + 1;
  state.lastRateLimitAt = new Date().toISOString();
  fs.writeFileSync(getStateFilePath(projectRoot), JSON.stringify(state, null, 2));
  _invalidateCache();
  return state;
}

module.exports = {
  getState,
  startLoop,
  incrementIteration,
  completeLoop,
  cancelLoop,
  isMaxReached,
  recordRateLimit,
  getStateFilePath,
  LOOP_DIR,
};
