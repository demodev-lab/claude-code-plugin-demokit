...
function extractScriptReferences(content) {
  ...
  const envPattern = /\$\{CLAUDE_PLUGIN_ROOT\}\/(scripts|hooks)\/([^\s"')`]+)/g;
  for (const match of content.matchAll(envPattern)) {
    refs.push(`${match[1]}/${match[2]}`);
  }

  const relativePattern = /(?:^|[\s("'`])(scripts|hooks)\/([^\s"')`]+)/g;
...
function resolveScriptCandidates(pluginRoot, scriptRef) {
  ...
  const normalized = (cleaned.startsWith('scripts/') || cleaned.startsWith('hooks/'))
    ? cleaned
    : `scripts/${cleaned}`;
...
function validateHookTimeout(stats, hook, eventName, entryIndex, hookIndex) {
  const location = `${eventName}[${entryIndex}].hooks[${hookIndex}]`;

  if (!Object.prototype.hasOwnProperty.call(hook, 'timeout')) {
    addWarning(stats, `Hook timeout missing: ${location} (expected timeout in milliseconds)`);
    return;
  }

  const timeout = hook.timeout;
  if (typeof timeout !== 'number' || !Number.isFinite(timeout)) {
    addError(stats, `Hook timeout must be a finite number: ${location}`);
    return;
  }

  if (timeout <= 0) {
    addError(stats, `Hook timeout must be > 0: ${location}`);
    return;
  }
  ...
  if (timeout < HOOK_TIMEOUT_MIN_MS) {
    addWarning(
      stats,
      `Hook timeout looks too small (possible seconds unit): ${location} = ${timeout}. Use milliseconds (e.g. 5000).`
    );
    return;
  }

  if (timeout > HOOK_TIMEOUT_MAX_MS) {
    addWarning(stats, `Hook timeout looks too large: ${location} = ${timeout}ms`);
  }
}
  ...
  for (const [eventName, entries] of Object.entries(hooks)) {
    ...
    entries.forEach((entry, entryIndex) => {
      ...
      entry.hooks.forEach((hook, hookIndex) => {
        ...
        validateHookTimeout(stats, hook, eventName, entryIndex, hookIndex);

        if (typeof hook.command !== 'string') {
          addWarning(stats, `Hook command missing: ${eventName}[${entryIndex}].hooks[${hookIndex}]`);
          return;
        }
