...
function buildTemplateSummary(hookData, projectRoot) {
  ...
  return {
    request: firstPrompt || contextTask || extractFirstMeaningful(text) || '(요약 없음)',
    investigated: [],
    learned: [],
    completed,
    next_steps: [],
...
function saveSummary(projectRoot, sessionData, summaryResult) {
  ...
  const doc = {
    ...
    prompts: enrichPromptsWithObservations(sessionData?.prompts || [], projectRoot),
  };
...
/**
 * 프롬프트별 observation 통계 첨부
 */
function enrichPromptsWithObservations(prompts, projectRoot) {
  ...
  try {
    ...
    for (const obs of observations) {
      ...
      if (!byPrompt[pn]) byPrompt[pn] = { files: new Set(), commands: [], toolUses: 0 };
      byPrompt[pn].toolUses++;
      if (obs.type === 'write' && obs.file) byPrompt[pn].files.add(path.basename(obs.file));
      if (obs.type === 'bash' && obs.command) byPrompt[pn].commands.push(obs.command.substring(0, 80));
    }
    return prompts.map(p => {
      const stats = byPrompt[p.number];
      if (!stats) return p;
      return { ...p, files: [...stats.files], toolUses: stats.toolUses };
    });
