...
function withPipelineLock(projectRoot, fn) {
  const statusFile = getStatusFile(projectRoot);
  io.ensureDir(path.dirname(statusFile));
  return io.withFileLock(statusFile, fn);
}

function startPipeline(projectRoot, feature, options = {}) {
  const { reset = false } = options;

  return withPipelineLock(projectRoot, () => {
...
function advancePipeline(projectRoot) {
  return withPipelineLock(projectRoot, () => {
    ...
    if (!next) {
      // 이미 완료된 최종 상태라면 history를 추가로 오염시키지 않고 그대로 반환
      if (state.completedAt && current.status === 'completed') {
        return {
          state,
          advanced: false,
          completed: true,
