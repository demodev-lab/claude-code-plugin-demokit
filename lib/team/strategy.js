...
/**
 * complexity mode를 반영한 실행 패턴 반환
 * @param {string} level
 * @param {string} phase
 * @param {string} complexityMode - 'single'|'team'
 * @returns {'single'|'leader'|'swarm'|'council'|'pipeline'|'watchdog'}
 */
function getPhaseExecutionPatternWithComplexity(level, phase, complexityMode) {
  if (complexityMode === 'single') return 'single';
  return getPhaseExecutionPattern(level, phase);
}
...
module.exports = {
  ...
  getPhaseExecutionPatternWithComplexity,
  getCtoAgent,
};
