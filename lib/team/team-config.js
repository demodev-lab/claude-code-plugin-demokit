...
function getTeamLevelOverridesFromConfig(cfg, level) {
  ...
  const candidateKeys = levelMapping.getCompatibleLevelKeys(level, cfg);
...
function getTeamProfile(level) {
  const cfg = loadConfigSafe();
  return levelMapping.resolveLevelProfile(level, cfg);
}
...
function resolvePhaseLevelValue(sectionMap, phase, levelCandidates) {
  ...
  const candidates = Array.isArray(levelCandidates)
    ? levelCandidates
    : [levelCandidates];

  for (const candidate of candidates) {
    if (Object.prototype.hasOwnProperty.call(phaseMap, candidate)) {
      return phaseMap[candidate];
    }
  }
...
function getPhasePerformanceValue(cfg, levelOverrides, section, phase, level) {
  const levelCandidates = levelMapping.getCompatibleLevelKeys(level, cfg);

  const overrideSectionMap = levelOverrides?.performance?.[section];
  const fromLevelOverride = resolvePhaseLevelValue(overrideSectionMap, phase, levelCandidates);
  ...
  return resolvePhaseLevelValue(sectionMap, phase, levelCandidates);
}
...
function getMaxTeammates(level) {
  ...
  for (const candidate of levelCandidates) {
    const fromCandidate = normalizePositiveInteger(maxMap[candidate]);
