...
function makeSetFromMembers(members) {
  const set = new Set();
  if (!Array.isArray(members)) return set;
  for (const member of members) {
    const memberId = normalizeMemberRef(member);
...
function normalizeMember(member) {
  ...
  if (!id) return null;
  const worktreePath = member.worktreePath || member.workingDirectory || member.cwd || null;
  return {
    id,
...
function normalizeState(state) {
  ...
  normalized.waveExecution = (normalized.waveExecution && typeof normalized.waveExecution === 'object')
    ? normalized.waveExecution
    : null;
  return normalized;
}
...
/**
 * taskQueue 동기화 로직(락 외부): 입력 state를 직접 변경
 */
function syncTaskQueueInState(state, options = {}) {
  const result = {
    updated: false,
    reason: 'no_change',
    state,
  ...
  if (hasFreshContext && shouldResetQueue) {
    const beforeTaskCount = Array.isArray(state.taskQueue) ? state.taskQueue.length : 0;
  ...
  if (
    ...
    const fallbackQueue = normalizeTaskQueue(options.taskQueue || []);
    if (hasFreshContext && fallbackQueue.length > 0) {
      state.taskQueue = fallbackQueue;
      result.updated = true;
      result.reason = 'task_queue_initialized';
      appendHistory(state, 'task_queue_initialized', {
        feature: targetFeature,
...
function cleanupStaleMembersInState(state, staleMemberMs) {
  const result = {
    state,
    removedMemberIds: [],
    changed: false,
  ...
  if (removedMembers.length === 0) return result;

  const removeSet = makeSetFromMembers(removedMembers);
  state.taskQueue = state.taskQueue.map((task) => {
    if (!task) return task;
    const assignee = normalizeMemberRef(task.assignee);
    if (!assignee || !removeSet.has(assignee)) return task;
    return {
      ...task,
      assignee: undefined,
      status: undefined,
  ...
  result.removedMemberIds = removedMembers;
  result.changed = true;
  return result;
}

function recordTaskCompletionInState(state, memberId, taskId, resultStatus, options = {}) {
  const hasTaskId = taskId !== undefined && taskId !== null;
  const now = nowString();
  const member = state.members.find(m => m.id === memberId);
  const worktreePath = options.worktreePath || member?.worktreePath || null;
  ...
  releaseTaskAssignmentsForMemberInState(state, memberId);
  ...
  return { state, removedTaskCount: 0, completed: true };
}

/**
 * Team 상태 mutation 공통 래퍼 (락/읽기/변경/저장)
 */
function withTeamStateMutation(projectRoot, mutateFn) {
  const statePath = getStatePath(projectRoot);
  return io.withFileLock(statePath, () => {
 ...
 * 멤버 상태 업데이트
 */
function updateMemberStatus(projectRoot, agentId, status, currentTask, options = {}) {
 ...
 * 작업 완료 기록
 */
function recordTaskCompletion(projectRoot, agentId, taskId, result, options = {}) {
  return io.withFileLock(getStatePath(projectRoot), () => {
    const state = loadTeamState(projectRoot);
    const hasTaskId = taskId !== undefined && taskId !== null;
    const now = nowString();
...
function syncTaskQueue(projectRoot, options = {}) {
  return io.withFileLock(getStatePath(projectRoot), () => {
    const state = loadTeamState(projectRoot);
    const result = syncTaskQueueInState(state, options);
    if (result.updated) {
      saveTeamState(projectRoot, state);
    }
    return state;
...
function cleanupStaleMembers(projectRoot, staleMemberMs = DEFAULT_STALE_MEMBER_MS) {
  return withTeamStateMutation(projectRoot, (state) => {
    const result = cleanupStaleMembersInState(state, staleMemberMs);
    return {
...
/**
 * 멤버 완료/큐 상태 반영 + stale 멤버 정리 + 큐 동기화 1회 수행
 */
function completeTaskAndSync(projectRoot, params = {}) {
  const {
    memberId,
    taskId,
    result,
  ...
  return withTeamStateMutation(projectRoot, (state) => {
    if (!state.enabled) {
      return { changed: false, reason: 'team_disabled' };
    }

    const staleResult = typeof staleMemberMs === 'number' || typeof staleMemberMs === 'string'
      ? cleanupStaleMembersInState(state, staleMemberMs)
      : { removedMemberIds: [], changed: false };

    let changed = staleResult.changed;

    let completionApplied = false;
    if (memberId) {
      recordTaskCompletionInState(state, memberId, taskId, result, { worktreePath });
      changed = true;
      completionApplied = true;
    }

    let taskQueueUpdated = false;
    if (syncContext && syncContext.taskQueue) {
      const syncResult = syncTaskQueueInState(state, syncContext);
      taskQueueUpdated = syncResult.updated;
      changed = changed || syncResult.updated;
    }

    return {
      changed,
      removedMemberIds: staleResult.removedMemberIds || [],
      completionApplied,
      taskQueueUpdated,
 ...
 * Wave 실행 상태 초기화
 */
function initWaveExecution(projectRoot, waveState) {
  return io.withFileLock(getStatePath(projectRoot), () => {
    const state = loadTeamState(projectRoot);
    state.waveExecution = waveState;
    saveTeamState(projectRoot, state);
    return state;
  });
}
 ...
 * Wave 실행 상태 업데이트
 */
function updateWaveExecution(projectRoot, updaterFn) {
  return io.withFileLock(getStatePath(projectRoot), () => {
    const state = loadTeamState(projectRoot);
    if (!state.waveExecution || typeof updaterFn !== 'function') {
      return state;
    }
    updaterFn(state.waveExecution);
    saveTeamState(projectRoot, state);
    return state;
  });
}
...
/**
 * 팀 상태 파일 lock 래퍼 (외부에서 atomic read-modify-write 시 사용)
 */
function withTeamLock(projectRoot, fn) {
  return io.withFileLock(getStatePath(projectRoot), fn);
}
