...
/**
 * config 캐시 (프로세스 내 TTL 10초)
 */
let _configCache = { projectRoot: null, config: null, expiresAt: 0 };

function _loadConfig(projectRoot) {
  const now = Date.now();
  if (_configCache.projectRoot === projectRoot && now < _configCache.expiresAt) {
  ...
  if (!config) {
    try {
      ...
      if (fs.existsSync(pluginConfig)) {
        config = JSON.parse(fs.readFileSync(pluginConfig, 'utf-8'));
      }
    } catch { /* ignore */ }
  }
  _configCache = { projectRoot, config, expiresAt: now + 10000 };
  return config;
}
...
function getAutomationLevel(projectRoot) {
  const config = _loadConfig(projectRoot);
 ...
 * @returns {boolean}
 */
function shouldAutoTransition(projectRoot, feature, currentPhase, currentStatus = null) {
  ...
  try {
    ...
    if (currentPhase === 'analyze') {
      const pdcaStatus = require('./status');
      const statusData = currentStatus || pdcaStatus.loadStatus(projectRoot, feature);
      const matchRate = statusData?.phases?.analyze?.matchRate;
      if (matchRate && typeof matchRate === 'object' && Number.isFinite(matchRate.totalRate)) {
