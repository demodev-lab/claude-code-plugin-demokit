...
const { debug: log, config } = require('../core');
...
const PHASE_DELIVERABLES = {
  ...
  do:      [{ name: 'Java files', pattern: 'src/main/java/**/entity/*.java' }],
...
const WALK_SKIP_DIRS = new Set(['.git', '.gradle', '.idea', '.cache', 'node_modules', 'build', 'target', 'out', 'dist', '.next']);

function getDoStickyCacheTtlMs() {
  const fromConfig = Number(config.getConfigValue('pdca.doStickyCacheTtlMs', 15000));
  if (!Number.isFinite(fromConfig) || fromConfig < 0) return 15000;
  return fromConfig;
}
...
function checkPhaseDeliverables(projectRoot, feature, phase) {
  ...
  // do phase는 완료 직후 짧은 구간에서 sticky cache를 허용해 반복 스캔 비용을 줄인다.
  // 단, TTL을 두어 코드 변경 이후 판정이 영구적으로 stale 되지 않도록 한다.
  if (cached && phase === 'do' && cached.result?.complete === true) {
    const ttlMs = getDoStickyCacheTtlMs();
    const ageMs = Date.now() - (cached.cachedAt || 0);
    if (ttlMs > 0 && ageMs <= ttlMs) {
      log.debug('pdca-phase', `deliverables sticky cache hit: ${phase}`, { feature, ageMs, ttlMs });
      return cloneDeliverableResult(cached.result);
    }
  }

  // do phase는 sticky TTL 이후 반드시 재평가하여 stale 판정을 줄인다.
  if (cached && phase !== 'do' && cached.signature === signature) {
  ...
  deliverableCache.set(cacheKey, {
    signature,
    cachedAt: Date.now(),
    result: cloneDeliverableResult(result),
  });
