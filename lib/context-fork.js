...
function mergeForkedContext(forkId, options = {}) {
  ...
  const parentState = JSON.parse(JSON.stringify(fork.parentState));
  const conflicts = [];

  for (const field of mergeFields) {
    if (fork.forkedState[field] === undefined) continue;

    // conflict 감지: parent가 변경되었는지
    const parentOriginal = JSON.stringify(fork.parentState[field]);
    const parentCurrent = JSON.stringify(parentState[field]);
    if (parentOriginal !== parentCurrent) {
      conflicts.push({
        field,
        parentValue: parentState[field],
    ...
    if (Array.isArray(fork.forkedState[field])) {
      ...
      parentState[field] = fork.forkedState[field];
    }
  }
  ...
  return { success: true, merged: parentState, conflicts, error: null };
}
