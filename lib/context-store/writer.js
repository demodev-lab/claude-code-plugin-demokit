...
function saveContext(projectRoot, { gradle, project, level, pdcaFeatures, loopState, recentChanges, currentTask, domains }) {
  ...
  const trimmedHistory = allHistory.slice(-MAX_HISTORY);
...
function appendLoopLog(projectRoot, { iteration, maxIterations, prompt, result, nextAction }) {
  ...
  try { fs.accessSync(filePath); fileExists = true; } catch { /* not found */ }
  if (!fileExists) {
    content += `# Loop 실행 로그\n`;
    content += `> 프롬프트: ${prompt}\n`;
  ...
  fs.appendFileSync(filePath, content + entry.join('\n'), 'utf-8');
  return filePath;
}
...
function appendChange(projectRoot, changeDescription) {
  ...
  // 이력 섹션 끝에 추가 (시간순 정렬 유지)
  const historyEntry = `- [${timestamp()}] ${changeDescription}`;
  if (content.includes('## 최근 변경 이력')) {
    // 다음 섹션(## ) 또는 파일 끝 앞에 삽입
    const historyIdx = content.indexOf('## 최근 변경 이력');
    const afterHistory = content.indexOf('\n## ', historyIdx + 1);
    if (afterHistory !== -1) {
      content = content.slice(0, afterHistory) + historyEntry + '\n' + content.slice(afterHistory);
    } else {
      // 마지막 섹션이면 끝에 추가
