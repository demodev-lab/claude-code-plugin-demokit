...
const { initWaveExecution, loadTeamState } = require('../team/state-writer');
...
const { LAYER_DEPENDENCIES, LAYER_FILE_PATTERNS } = require('../team/layer-constants');
...
const TEAM_THRESHOLD = 30;

function assessComplexity(requestText, signals, size, level) {
  let score = 0;
  const activeSignals = Object.values(signals).filter(Boolean).length;
  score += activeSignals * 10;
  if (size.size === 'majorFeature') score += 30;
  else if (size.size === 'feature') score += 20;
  else if (size.size === 'minorChange') score += 5;
  const levelWeights = { Starter: 0, SingleModule: 5, MultiModule: 15, Monolith: 20, MSA: 25 };
  score += levelWeights[level] || 0;
  if (level === 'Starter' || score < TEAM_THRESHOLD) {
    const reason = level === 'Starter' ? 'starter_level' : 'low_complexity';
    return { mode: 'single', score, reason };
  }
  return { mode: 'team', score, reason: 'complexity_exceeds_threshold' };
}

function uniqueTaskList(tasks) {
  // 1) title 기준 중복 제거
  const seenTitle = new Set();
  const titleUnique = tasks.filter((task) => {
    const normalized = safeTrim(task.title).toLowerCase();
    if (seenTitle.has(normalized)) return false;
    seenTitle.add(normalized);
    return true;
  });

  // 2) layer 기준 병합 — 같은 layer의 추가 task는 첫 task에 title 합침
  const layerMap = new Map();
  for (const task of titleUnique) {
...
function buildPrdSection({ requestText, featureSlug, size, signals }) {
  const lines = ['# PRD'];
  lines.push('');
  lines.push('## 요약');
  lines.push(`- **요청**: ${requestText || '(없음)'}`);
  lines.push(`- **feature**: \`${featureSlug}\``);
  lines.push(`- **규모**: ${size.label}`);
  lines.push('');
  lines.push('## 감지된 신호');
  const active = Object.entries(signals).filter(([, v]) => v).map(([k]) => k);
  lines.push(active.length > 0 ? active.map(s => `- ${s}`).join('\n') : '- (없음)');
  lines.push('');
  lines.push('## 수락 기준');
  lines.push('- [ ] 모든 Do 단계 task 완료');
  lines.push('- [ ] Analyze 점수 90% 이상');
  if (active.includes('security')) lines.push('- [ ] 보안 검증 통과');
  if (active.includes('api')) lines.push('- [ ] API 스펙 문서화');
  return lines.join('\n');
}

function buildDesignSection({ phases }) {
  const lines = ['# DESIGN'];
  const designPhase = phases.find(p => p.id === 'design');
  if (designPhase) {
    lines.push('');
    lines.push(`## ${designPhase.title}`);
    lines.push(`**목표**: ${designPhase.goal}`);
    if (designPhase.tasks && designPhase.tasks.length > 0) {
      lines.push('');
      lines.push('### 설계 항목');
      for (const task of designPhase.tasks) {
  ...
  if (doPhase && doPhase.tasks && doPhase.tasks.length > 0) {
    const usedLayers = [...new Set(doPhase.tasks.map(t => t.layer).filter(Boolean))];
    if (usedLayers.length > 1) {
      lines.push('');
      lines.push('### Layer 의존관계');
      lines.push('```');
      for (const layer of usedLayers) {
        const deps = (LAYER_DEPENDENCIES[layer] || []).filter(d => usedLayers.includes(d));
        if (deps.length > 0) {
          lines.push(`${deps.join(', ')} → ${layer}`);
        } else {
          lines.push(`${layer} (독립)`);
        }
      }
...
function buildTasksSection({ phases, featureSlug }) {
  const lines = ['# TASKS'];
  const doPhase = phases.find(p => p.id === 'do');
  if (doPhase) {
    for (const task of doPhase.tasks) {
      const deps = LAYER_DEPENDENCIES[task.layer] || [];
      const depStr = deps.length > 0 ? ` (depends: ${deps.join(', ')})` : '';
      lines.push(`- [ ] ${task.title} [${task.layer}]${depStr}`);
    }
    if (doPhase.parallelGroups?.length > 1) {
      lines.push('## 병렬 그룹');
      doPhase.parallelGroups.forEach((group, idx) => {
        lines.push(`- Wave ${idx + 1}: ${group.map(t => t.layer).join(', ')}`);
      });
    }
  }
  return lines.join('\n');
...
function buildTasksYaml({ phases, featureSlug }) {
  const doPhase = phases.find(p => p.id === 'do');
  if (!doPhase || !Array.isArray(doPhase.tasks) || doPhase.tasks.length === 0) {
    return { version: '1.0', feature: featureSlug, tasks: [], waves: [] };
  }

  // 1st pass: ID 생성 + layer→id 맵 구축
  const taskIdMap = new Map();
  const tasks = doPhase.tasks.map((task, idx) => {
    const layer = task.layer || 'unknown';
    const id = `${layer}-${idx + 1}`;
    taskIdMap.set(layer, [...(taskIdMap.get(layer) || []), id]);
    const ownFiles = LAYER_FILE_PATTERNS[layer];
    return {
      id,
      title: task.title,
      layer,
      ...
      dependsOn: [],
      ...(ownFiles ? { ownFiles: [...ownFiles] } : {}),
    };
  });

  // 2nd pass: 모든 ID가 등록된 후 dependsOn 계산
  for (const task of tasks) {
    task.dependsOn = (LAYER_DEPENDENCIES[task.layer] || [])
      .flatMap(depLayer => taskIdMap.get(depLayer) || [])
      .filter(depId => depId !== task.id);
  }

  const waves = (doPhase.parallelGroups || []).map((group, idx) => ({
    index: idx + 1,
    layers: group.map(t => t.layer).filter(Boolean),
  }));

  for (const task of tasks) {
    const waveIdx = waves.findIndex(w => w.layers.includes(task.layer));
    task.wave = waveIdx >= 0 ? waveIdx + 1 : null;
  }
...
function serializeYaml(obj) {
  const lines = [];
  lines.push(`version: "${escapeYamlString(obj.version)}"`);
  lines.push(`feature: "${escapeYamlString(obj.feature)}"`);
  if (!obj.tasks || obj.tasks.length === 0) {
    lines.push('tasks: []');
  } else {
    lines.push('tasks:');
    for (const task of obj.tasks) {
      lines.push(`  - id: "${escapeYamlString(task.id)}"`);
      lines.push(`    title: "${escapeYamlString(task.title)}"`);
      lines.push(`    layer: ${task.layer}`);
      lines.push(`    owner: ${task.owner != null ? task.owner : 'null'}`);
      lines.push(`    wave: ${task.wave != null ? task.wave : 'null'}`);
      if (!task.dependsOn || task.dependsOn.length === 0) {
        lines.push('    dependsOn: []');
      } else {
        lines.push('    dependsOn:');
        for (const dep of task.dependsOn) {
          lines.push(`      - "${escapeYamlString(dep)}"`);
        }
      }
      if (task.ownFiles && task.ownFiles.length > 0) {
        lines.push('    ownFiles:');
        for (const pattern of task.ownFiles) {
  ...
  if (!obj.waves || obj.waves.length === 0) {
    ...
    for (const wave of obj.waves) {
      lines.push(`  - index: ${wave.index}`);
      if (!wave.layers || wave.layers.length === 0) {
        lines.push('    layers: []');
      } else {
        lines.push('    layers:');
...
function buildMarkdown({ requestText, featureSlug, size, phases, startedWaveState, complexity }) {
  const lines = [];
  ...
  if (complexity) {
    lines.push(`- 실행 모드: ${complexity.mode === 'single' ? 'Single Agent' : 'Team (Wave 병렬)'} (score: ${complexity.score})`);
  }
  lines.push('');
  ...
  if (doPhase) {
    ...
    if (complexity?.mode === 'team' && doPhase.team && doPhase.team.enabled && doPhase.parallelGroups && doPhase.parallelGroups.length > 1) {
...
function buildSuperworkBlueprint(rawPrompt) {
  ...
  const complexity = assessComplexity(parsed.requestText, signals, size, level);
  ...
  if (complexity.mode === 'team' && doPhase?.team?.enabled && doPhase?.parallelGroups?.length > 1) {
    ...
    if (wavePlan.length > 0) {
      try {
        ...
        if (projectRoot) {
          // 이미 활성 waveExecution이 있으면 중복 초기화 방지
          const existingState = loadTeamState(projectRoot);
          if (existingState.waveExecution && existingState.waveExecution.status !== 'completed') {
            startedWaveState = existingState.waveExecution;
          } else {
  ...
  const tasksYamlObj = buildTasksYaml({ phases, featureSlug: parsed.featureSlug });
  const tasksYamlStr = serializeYaml(tasksYamlObj);

  const artifacts = {
    prd: buildPrdSection({ requestText: parsed.requestText, featureSlug: parsed.featureSlug, size, signals }),
    design: buildDesignSection({ phases }),
    tasks: buildTasksSection({ phases, featureSlug: parsed.featureSlug }),
    tasksYaml: tasksYamlStr,
  };
  ...
  try {
    const projectRoot = platform.findProjectRoot(process.cwd());
    if (projectRoot) {
      const { saveArtifacts } = require('./artifact-writer');
      saveArtifacts(projectRoot, parsed.featureSlug, artifacts);
    }
  } catch (_) { /* 저장 실패해도 blueprint 반환 */ }
