...
function buildTeamContext(level, phaseId, feature) {
  ...
  return {
    ...
    maxParallel: context.maxParallel || 1,
    delegateMode: Boolean(context.delegateMode),
    delegateAgent: context.delegateAgent || null,
...
function buildPdcaFlow(featureSlug, size) {
  ...
  if (size?.size === 'feature' || size?.size === 'majorFeature') {
...
function buildMarkdown({ requestText, featureSlug, size, phases }) {
  const lines = [];
  ...
  lines.push('아래 순서로 PDCA를 붙이면 구현, 분석, 재작업이 자동으로 이어집니다.');
