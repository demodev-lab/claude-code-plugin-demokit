...
const { buildWavePlan, buildWaveExecutionMarkdown, createWaveState } = require('../team/wave-executor');
  ...
  if (doPhase) {
    ...
    if (doPhase.team && doPhase.team.enabled && doPhase.parallelGroups && doPhase.parallelGroups.length > 1) {
      ...
      if (wavePlan.length > 0) {
        lines.push('');
        lines.push(buildWaveExecutionMarkdown(wavePlan, featureSlug));
      }
    }
  }
