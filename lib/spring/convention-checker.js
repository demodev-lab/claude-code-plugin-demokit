...
function checkForbiddenPatterns(content, layerType) {
  ...
  if (content.match(/new\s+RestTemplate\s*\(/) || content.match(/RestTemplate\s+\w+\s*[=;]/)) {
    violations.push('RestTemplate 사용 금지 → WebClient');
  }
  if (/\bRestClient\b/.test(content) && !content.includes('MockRestServiceServer') && !content.includes('RestClientTest')) {
...
function checkAdvancedPatterns(content, layerType) {
  ...
  if (layerType === 'entity') {
    const toManyMatches = [...content.matchAll(/@(OneToMany|ManyToMany)\s*(\([^)]*\))?/g)];
    for (const match of toManyMatches) {
      if (!match[0].includes('FetchType.LAZY')) {
