...
function extractBlock(content, keyword) {
  ...
  return depth === 0 ? content.slice(open + 1, i - 1) : null;
}
...
function extractPlugins(content) {
  const plugins = [];
  const block = extractBlock(content, 'plugins');
  if (!block) return plugins;
...
function extractDependencies(content) {
  const deps = [];
  const block = extractBlock(content, 'dependencies');
...
function extractRepositories(content) {
  const repos = [];
  const block = extractBlock(content, 'repositories');
  ...
  if (urlMatch) {
    urlMatch.forEach(u => {
      const innerMatch = u.match(/["'](.+?)["']/);
      if (innerMatch) repos.push(innerMatch[1]);
    });
  }
  return repos;
}
