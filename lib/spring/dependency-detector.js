...
/**
 * 의존성 목록에서 프로젝트 특성 감지
 * @param {Array} dependencies - 의존성 목록 (문자열 또는 {raw, artifact} 객체)
 * @returns {Object[]} 감지된 특성 목록
 */
function detectFeatures(dependencies) {
  const depsLower = dependencies.map(d =>
    (typeof d === 'string' ? d : d.raw || d.artifact || '').toLowerCase()
  );
  const detected = [];
...
function checkQuerydslFork(dependencies) {
  const toStr = d => typeof d === 'string' ? d : d.raw || d.artifact || '';
  const hasOldQuerydsl = dependencies.some(d =>
    toStr(d).includes('com.querydsl') && !toStr(d).includes('openfeign')
  );
  const hasNewQuerydsl = dependencies.some(d =>
    toStr(d).includes('io.github.openfeign.querydsl')
  );
